  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
      {MOCK_LAPORAN_METRICS.map((m) => {
        const isActive = timePeriod === m.period;
        const bgClass = isActive ? "bg-green" : "bg-gray-200 dark:bg-gray-600";
        ...
        return (
          <div
            key={m.id}
            className={`rounded-[10px] border-0 p-6 ${bgClass} ${textClass}`}
          >
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div
                  className={`mb-1 text-xs sm:text-sm md:text-xs ${isActive ? "text-white/80" : "text-gray-500 dark:text-gray-400"}`}
                >
                  {m.title}
                </div>
